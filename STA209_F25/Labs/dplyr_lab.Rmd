...
**Question 1:** Filter the college dataset to include only schools located in the Plains and the Great Lakes regions and with enrollments less than 20,000. Using your filtered data, create a two-way table using the variables `Region` and `Type`. **Hint**: `with(table())`
...
ggplot(tt, aes(fill = Size, y = Region)) + geom_bar(position = "fill") +
  labs(x = "Proportion")
```
