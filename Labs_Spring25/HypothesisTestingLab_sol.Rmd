---
title: "Hypothesis Testing Lab Solutions"
output:
...
We changed the value we are testing for the mean in the hypothesis. This changes the numerator of the test-statistic.
...
They are pretty much the same values for sample means.
...
We get strong evidence the mean is not equal to 1050 for the original dataset. Little to none for the smaller data set.

**Hawk 2C:** Explain why the conclusions you came to were different in Part B.

Even though the sample means and standard deviations are the same, the sample size is much larger for the first group, and so the test-stat is also larger $\rightarrow$ smaller p-value.
...
We get moderate evidence the means are not the same.
...
Outliers gave us a much larger p-value, and thus weaker evidence. This is because outliers increase variability and so the sample standard deviations were larger. Removing them thus gives us larger test-stats (we are dividing by a smaller number) and smaller p-values.
...
We get a p-value of 0.9736. We were trying to find out if number of deaths we get for each corp-year matched what is expected under a Poisson distribution. The null hypothesis is that the proportions of deaths match the poisson distribution, and so we have little to no evidence that they don't. It seems like Poisson distribution works well for death-by-mule kick (morbidly funny wording).
...
We get an extremely small p-value. We have overwhelming evidence that Region and Private/Public are associated with each other. Visually, not all regions had the same proportions of Public and Private colleges.